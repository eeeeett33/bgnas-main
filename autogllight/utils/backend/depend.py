...
class _BackendConfig(_typing.Mapping[str, _typing.Any]):
    def __init__(
        self, name: str, configurations: _typing.Mapping[str, _typing.Any] = ...
    ):
        self.__name: str = name
        if configurations not in (None, Ellipsis, ...) and isinstance(
            configurations, _typing.Mapping
        ):
...
class _DependentBackendMetaclass(type):
    ...
    def __new__(
        ...
        instance = super(_DependentBackendMetaclass, mcs).__new__(

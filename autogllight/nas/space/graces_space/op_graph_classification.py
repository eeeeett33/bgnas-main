...
class NaAggregator(nn.Module):
  ...
  def __init__(self, in_dim, out_dim, aggregator):
    ...
    if 'arma' == aggregator:
      self._op = ARMAConv(in_dim, out_dim)
    if 'gat' == aggregator:
      heads = 2
      out_dim /= heads
    ...
    if aggregator =='mlp':
      self._op = Sequential(Linear(in_dim, out_dim), ELU(), Linear(out_dim, out_dim))
        ...
        self._op = GCNConvMol(in_dim)
    elif aggregator =='gatmol':
        self._op = GATConvMol(in_dim)
    elif aggregator =='ginmol':
        self._op = GINConvMol(in_dim)
    elif aggregator =='sagemol':
        self._op = SAGEConvMol(in_dim)
    elif aggregator =='graphmol':
        self._op = GraphConvMol(in_dim)
    elif aggregator =='mlpmol':
        self._op = MLPConvMol(in_dim)
  ...
  def forward(self, x, edge_index, edge_weight=None, edge_attr=None):
    if self.aggregator == 'mlp':
      ...
      return self._op(x, edge_index, edge_attr=edge_attr)
    else:
...
class Readout_func(nn.Module):
  def __init__(self, readout_op, hidden):
    ...
    if readout_op == 'mean':
      ...
      self.k = 10
      self.sort_conv = Conv1d(hidden, hidden, 5)

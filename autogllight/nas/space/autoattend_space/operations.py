...
import numpy as np
from ..operation import GeoLayer

class AggAdd(nn.Module):
    def __init__(
        self, dim, att_head, dropout=0, norm=False, skip_connect=False, *args, **kwargs
    ):
...
agg_map = {
    "add": lambda dim, att_head=None, dropout=0, norm=False, skip_connect=False: AggAdd(
        dim, att_head, dropout, norm, skip_connect
    ),
    "attn": lambda dim, att_head=None, dropout=0, norm=False, skip_connect=False: AggAttn(
        dim, att_head, dropout, norm, skip_connect
    ),
}

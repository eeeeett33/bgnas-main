...
class AutoAttendNodeClassificationSpace(BaseSpace):
    ...
    def __init__(
        ...
        assert not is_dgl(), "Now AutoAttend only support pyg"
    ...
    def build_graph(self):
        ...
        self.gnn_map = lambda x, *args, **kwargs: OPS[self.ops_type][x](*args, **kwargs)
        self.gnn_ops = self.gnn_ops or PRIMITIVES
        self.agg_map = lambda x, *args, **kwargs: agg_map[x](*args, **kwargs)
    ...
    def _set_agg_choice(self, layer, key):
        ...
        ops = [self.agg_map(op, dim, agg_head, self.dropout) for op in self.agg_ops]
        choice = self.setLayerChoice(layer, ops, key=key,)
        return choice
    ...
    def _set_layer_choice(self, layer, key):
        ...
        if self.ops_type == 0:
            ops = [
                self.gnn_map(op, input_dim, output_dim, self.dropout, concat=concat)
                for op in opnames
            ]
        elif self.ops_type == 1:
            ops = [
                self.gnn_map(
                    op, input_dim, output_dim, self.head, self.dropout, concat=concat
                )
                for op in opnames
            ]
        choice = self.setLayerChoice(l1, ops, key=key,)
        return choice

    def _set_input_choice(self, key, layer, **kwargs):
        self.setInputChoice(layer, key=key, **kwargs)
    ...
    def forward(self, data):
        ...
        for layer in range(1, self.layer_number + 1):
            ...
            stem_out = BK.gconv(op, data, drop(input))
            ...
            for l2 in range(layer, self.layer_number + 1):
                ...
                for i in range(2):
                    ...
                    side_out = BK.gconv(op, data, drop(input))
